...
mentioning an interesting concept I recently came across -- PECS or "Producer
...
- Use `? extends T` when the collection is "producing" items that you utilize. Like,
  ...
  compatible with `T` (i.e, sub-classes of `T`). This is covariance -- narrowing of
  scope.
- Use `super` when collection is "comnsuming" items. Like, an `copyToList` method
  ...
  `T`. This is contra-variance -- widening of scope.
